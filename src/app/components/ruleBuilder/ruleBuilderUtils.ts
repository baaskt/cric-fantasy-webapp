...
export const emptyThreshold = (): Threshold => ({ id: uid(), value: 0, operator: 'gte', points: 0 })
...
export const emptyRule = (): ScoredSegmentsRule => ({
  id: uid(),
  name: 'newRule',
  type: 'scored_segments',
  segments: [emptySegment()],
})
export const emptyGroupRule = (): GroupRule => ({
  id: uid(),
  name: 'impact',
  type: 'group',
  rules: [],
})
export const emptyChildRule = (): ScoredSegmentsRule => ({
  id: uid(),
  name: 'newChild',
  type: 'scored_segments',
  segments: [emptySegment()],
...
export function buildFormulaPreview(score: ScoreOp): FormulaPreviewResult | null {
  ...
  if (op === 'difference') {
    const fields = score.fields ?? []
    if (!fields.length) return null
    const parts: FormulaPart[] = [{ t: fields[0] ?? '', c: 'var(--accent2)' }]
...
export function initModeData(defaultRules: DefaultRulesConfig): ModeData {
  return Object.fromEntries(
    Object.entries(MODES).map(([mode, { categories }]) => {
      const base: Category[] = categories.map(name => ({ id: uid(), name, rules: [] }))
      return [mode, parseJsonToCategories(defaultRules[mode as ModeName] ?? {}, base)]
    }),
  ) as ModeData
}

export function parseJsonToModes(parsed: DefaultRulesConfig, existing: ModeData): ModeData {
  return Object.fromEntries(
    Object.entries(existing).map(([mode, cats]) => [
      mode,
      parseJsonToCategories(parsed[mode as ModeName] ?? {}, cats),
    ]),
  ) as ModeData
}

export function parseJsonToCategories(parsed: RawModeConfig, existing: Category[]): Category[] {
  const parseRule = (name: string, def: RawRule): Rule => {
    if (def.type === 'group') {
      return {
        id: uid(),
        name,
        type: 'group',
        rules: Object.entries(def.rules ?? {}).map(
          ([cn, cd]) => parseRule(cn, cd) as ScoredSegmentsRule,
        ),
      }
    }
    return {
      id: uid(),
      name,
      type: def.type ?? 'scored_segments',
      segments: (def.segments ?? []).map(seg => ({
        id: uid(),
        label: seg.label ?? 'Segment',
        when: (seg.when ?? []).map(c => ({ id: uid(), ...c })),
        score: seg.score ?? ({ operation: 'field', field: 'runs' } as ScoreOp),
...
const serialiseScore = (s: ScoreOp): ScoreOp => {
  if (s.operation === 'fixed') return { operation: 'fixed', value: s.value ?? 0 }
  return s
}

const serialisePoints = (p: PointsValue): PointsValue => {
  if (typeof p === 'object' && p !== null) return p
  return Number(p ?? 0)
}

const serialiseRule = (rule: Rule): object => {
  if (rule.type === 'group') {
    return {
      type: 'group',
...
export function buildRuleJson(rule: Rule): string {
  return JSON.stringify({ [rule.name]: serialiseRule(rule) }, null, 2)
}

export function buildJson(modeData: ModeData): string {
  const out: Record<string, Record<string, Record<string, object>>> = {}
  for (const [mode, cats] of Object.entries(modeData)) {
    const modeOut: Record<string, Record<string, object>> = {}
    for (const cat of cats) {
      if (!cat.rules.length) continue
      modeOut[cat.name] = {}
      for (const rule of cat.rules) {
        modeOut[cat.name][rule.name] = serialiseRule(rule) as Record<string, object>
      }
    }
